 ...
 *****************************************************************************/
 ...
 05/18/11   pgw     Changed to uint64_t use local handle rather than passing it in
...
int DiskWriter::GetDiskInfo(disk_entry_t* de) {
    ...
    swprintf_s(tPath, _T("\\\\.\\PhysicalDrive%d"), de->disknum);
...
int DiskWriter::InitDiskList(bool verbose) {
        ...
        for (int i = 0;; i++) {
            ...
            if (SetupDiGetDeviceProperty(hDevInfo, &DeviceInfoData, &DEVPKEY_Device_FriendlyName, &ulPropertyType, (BYTE*) szBuffer, sizeof(szBuffer), &dwSize, 0)) {
                if ((GetLastError() == ERROR_SUCCESS) && wcsstr(szBuffer, L"QDLoader 9008") != NULL) {
                    // wprintf(szBuffer);
                    LDEBUG("DiskWriter::InitDiskList", "发现edl设备: %s", string_utils::wstr2str(szBuffer).c_str());
                    ...
                    if (verbose) {
                        ...
                        if (port != NULL) {
                            ...
                            if (status == ERROR_SUCCESS) {
                                ...
                                LWARN("DiskWriter::InitDiskList", " => 无法读取设备信息" + string_utils::wstr2str(getErrorDescription(status)));
                            }
                        }
                    }
                    wprintf(_T("\n"));
    ...
    for (i = 0; i < MAX_DISKS; i++) {
        ...
        if (disks[i].disknum != -1) {
            wprintf(L"%d. %ls  Size: %lldMB, (%lld sectors), size: %d Mount:%ls, Name:[%ls]\n", 
                i, 
                disks[i].diskname, 
...
int DiskWriter::CorruptionTest(uint64_t offset) {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
        ...
        if (!bWriteDone && (status != ERROR_SUCCESS)) {
            wprintf(L"status %d bWriteDone %d bytesWrite %d\n", status, (int) bWriteDone, (int) bytesOut);
            break;
        }
        ...
        if (!bReadDone && (status != ERROR_SUCCESS)) {
            wprintf(L"status %d bReadDone %d bytesRead %d\n", status, (int) bReadDone, (int) bytesOut);
            break;
        }
        ...
        for (int i = 0; i < MAX_TEST_SIZE; i++) {
            if (temp1[i] != (char) i) {
                ...
                wprintf(L"Failed: Offset:  at %d expected %d found %d\n", i, (i & 0xff), (int) temp1[i]);
            }
        }
        wprintf(L".");
        offset += 0x40;
...
int DiskWriter::DiskTest(uint64_t offset) {
    ...
    wprintf(L"Sequential write test 4MB buffer %d\n", (uint64_t) temp1);
    ovl.Offset = (offset & 0xffffffff);
    ovl.OffsetHigh = (offset >> 32);
    ...
    for (int i = 0; i < 50; i++) {
        ...
        if (!bWriteDone)
            status = GetLastError();
        if (!bWriteDone && (status != ERROR_SUCCESS)) {
            wprintf(L"status %d bWriteDone %d bytesOut %d\n", status, (int) bWriteDone, (int) bytesOut);
            break;
        }
    }
    ticks = GetTickCount64() - ticks;
    wprintf(L"Sequential Write transfer rate: %d KB/s\n", (int) (50 * MAX_TEST_SIZE / ticks));
    ...
    for (iops = 0; (GetTickCount64() - ticks) < 2000; iops++) {
        ...
        if (!bWriteDone)
            status = GetLastError();
        if (!bWriteDone && (status != ERROR_SUCCESS)) {
            wprintf(L"status %d Offset: %d bWriteDone %d bytesWrite %d\n", status, (uint32_t) ovl.Offset, (int) bWriteDone, (int) bytesOut);
            break;
        }
        ovl.Offset += 0x1000;
    }
    ticks = GetTickCount64() - ticks;
    wprintf(L"Random 4K write IOPS = %d\n", (int) (iops / 2));
    ...
    wprintf(L"Sequential read test 4MB buffer %d\n", (uint64_t) temp1);
    ovl.Offset = (offset & 0xffffffff);
    ovl.OffsetHigh = (offset >> 32);
    ticks = GetTickCount64();
    for (int i = 0; i < 50; i++) {
    ...
    ticks = GetTickCount64() - ticks;
    wprintf(L"Sequential Read transfer rate: %d KB/s in %d ms\n", (int) (50 * MAX_TEST_SIZE / ticks), (int) ticks);
    ...
    wprintf(L"Random 4K read IOPS = %d\n", (int) (iops / 2));
...
int DiskWriter::UnmountVolume(vol_entry_t vol) {
    ...
    if (hVolume != INVALID_HANDLE_VALUE) {
        ...
        LDEBUG("DiskWriter::UnmountVolume", "成功卸载卷%s");
    } else {
        // wprintf(L"Warning: Failed to unmount drive continuing...\n");
        LWARN("DiskWriter::UnmountVolume", "无法卸载卷%s", string_utils::wstr2str(volPath).c_str());
    }
    //} catch (exception &e) {
...
int DiskWriter::OpenDiskFile(TCHAR* oFile, uint64_t sectors) {
    int status = ERROR_SUCCESS;
    if (oFile == NULL) {
        return ERROR_INVALID_HANDLE;
    }
...
int DiskWriter::OpenDevice(int dn) {
    ...
    if (de.volnum[0] != 0) {
        if (UnmountVolume(volumes[de.volnum[0]]) != ERROR_SUCCESS) {
            // wprintf(L"Warning: Failed to unmount volume %ls\n", volumes[de.volnum[0]].mount);
            LWARN("DiskWriter::OpenDevice", "无法卸载卷%s", string_utils::wstr2str(volumes[de.volnum[0]].mount).c_str());
            // return ERROR_INVALID_HANDLE;
        }
    }
    ...
    swprintf_s(tPath, _T("\\\\.\\PhysicalDrive%d"), dn);
    // wprintf(L"Using drive %ls\n", tPath);
    LDEBUG("DiskWriter::OpenDevice", "正在使用磁盘 %s\n", string_utils::wstr2str(tPath).c_str());
    // Create the file using the volume name
    hDisk = CreateFile(tPath,
        GENERIC_WRITE | GENERIC_READ,
...
int DiskWriter::RawReadTest(uint64_t offset) {
    ...
    if (!bResult) {
        ...
        if (dwResult == ERROR_IO_PENDING) {
            ...
            wprintf(L"Wait for it to finish %lld\n", offset);
...
int DiskWriter::FastCopy(HANDLE hRead, int64_t sectorRead, HANDLE hWrite, int64_t sectorWrite, uint64_t sectors, uint8_t partNum) { // Set up the overlapped structure
    ...
    while (sec < sectors) {
        ...
        wprintf(L"Sectors remaining: %d      \r", (int) (sectors - sec));
    }
    wprintf(L"\nStatus = %d\n", status);
