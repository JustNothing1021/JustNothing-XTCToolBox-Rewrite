...
int Dload::ConnectToFlashProg(BYTE ver) {
    BYTE hello[] = { EHOST_HELLO_REQ,
        'Q','C','O','M',' ','f','a','s','t',' ','d','o','w','n','l','o','a','d',' ','p','r','o','t','o','c','o','l',' ','h','o','s','t',
        ver, 2, 1 };
...
int Dload::LoadPartition(TCHAR* szPrtnFile) {
    ...
    hFile = CreateFile(szPrtnFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
int Dload::LoadImageFile(TCHAR* szSingleImage) {
    HANDLE hFile;
    int status = ERROR_SUCCESS;
...
int Dload::LoadFlashProg(TCHAR* szFlashPrg) {
    ...
    while (!feof(fMprgFile)) {
        ...
        if (bytesRead > 0) {
            ...
            rspSize = sizeof(rsp);
            //wprintf(L"Program at: 0x%x length %d\n",targetAddr,bytesRead);
