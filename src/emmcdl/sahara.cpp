...
int Sahara::LoadFlashProg(TCHAR* szFlashPrg) {
    ...
    // Log(L"Successfully open flash programmer to write: %ls\n", szFlashPrg);
    LDEBUG("Sahara::LoadFlashProg", "成功打开烧录内核文件: \"%s\"", string_utils::wstr2str(szFlashPrg).c_str());
...
int Sahara::ConnectToDevice(bool bReadHello, int mode) {
    ...
    if (status != ERROR_SUCCESS) {
        // Log("Failed to write hello response back to device (%d)\n", status);
