...
void FFUImage::SetOffset(OVERLAPPED* ovlVariable, uint64_t offset) {
...
int FFUImage::AddEntryToRawProgram(const char* szRawProgram, const char* szFileName, uint64_t ui64FileOffset, int64_t i64StartSector, uint64_t ui64NumSectors) {
    HANDLE hRawPrg;
    char szXMLString[MAX_STRING_LEN * 2];
    char szFile[MAX_STRING_LEN];
    DWORD dwBytesWrite;
    int status = ERROR_SUCCESS;
    hRawPrg = CreateFileA(szRawProgram,
    ...
    if (hRawPrg == INVALID_HANDLE_VALUE) {
        ...
        strcpy_s(szFile, sizeof(szFile), szFileName);
        if (i64StartSector < 0) {
            sprintf_s(szXMLString, sizeof(szXMLString), "<program SECTOR_SIZE_IN_BYTES=\"%d\" file_sector_offset=\"%lld\" filename=\"%s\" label=\"ffu_image_%lld\" num_partition_sectors=\"%lld\" physical_partition_number=\"0\" size_in_KB=\"%lld\" sparse=\"false\" start_byte_hex=\"0x%llx\" start_sector=\"NUM_DISK_SECTORS%lld\"/>\n",
                DISK_SECTOR_SIZE, ui64FileOffset, szFile, i64StartSector, ui64NumSectors, ui64NumSectors / 2, i64StartSector * 512, i64StartSector);
        } else {
            sprintf_s(szXMLString, sizeof(szXMLString), "<program SECTOR_SIZE_IN_BYTES=\"%d\" file_sector_offset=\"%lld\" filename=\"%s\" label=\"ffu_image_%lld\" num_partition_sectors=\"%lld\" physical_partition_number=\"0\" size_in_KB=\"%lld\" sparse=\"false\" start_byte_hex=\"0x%llx\" start_sector=\"%lld\"/>\n",
                DISK_SECTOR_SIZE, ui64FileOffset, szFile, i64StartSector, ui64NumSectors, ui64NumSectors / 2, i64StartSector * 512, i64StartSector);
        }
...
int FFUImage::TerminateRawProgram(const char* szFileName) {
    HANDLE hRawPrg;
    DWORD dwBytesOut;
    char szXMLString[] = "</data>\n";
    hRawPrg = CreateFileA(szFileName,
        GENERIC_WRITE | GENERIC_READ,
        0,    // We want exclusive access to this disk
        NULL,
        OPEN_EXISTING,
...
int FFUImage::CreateRawProgram(const char* szFFUFile, const char* szFileName) {
    HANDLE hRawPrg;
    DWORD dwBytesOut;
    char szPath[256] = { 0 };
    char szOut[256] = { 0 };
    ...
    strcpy_s(szPath, sizeof(szPath), szFFUFile);
    strcpy_s(szOut, sizeof(szOut), szFileName);
...
int FFUImage::ParseHeaders(void) {
    ...
    if ((FFUStoreHeader.dwValidateDescriptorLength < 1000000) &&
        ...
        if (ValidationEntries == NULL || BlockDataEntries == NULL) {
            LERROR("FFUImage::ParseHeaders", "无法为头部分配内存");
...
int FFUImage::DumpRawProgram(const char* szFFUFile, const char* szRawProgram) {
    ...
    for (uint32_t i = 0; i < FFUStoreHeader.dwWriteDescriptorCount; i++) {
        for (uint32_t j = 0; j < BlockDataEntriesPtr->dwLocationCount; j++) {
        ...
        if (BlockDataEntriesPtr->dwLocationCount > 0) {
            BlockDataEntriesPtr = (BLOCK_DATA_ENTRY*) ((uint64_t) BlockDataEntriesPtr + (sizeof(BLOCK_DATA_ENTRY) + (BlockDataEntriesPtr->dwLocationCount - 1) * sizeof(DISK_LOCATION)));
        } else {
            BlockDataEntriesPtr = (BLOCK_DATA_ENTRY*) ((uint64_t) BlockDataEntriesPtr + sizeof(BLOCK_DATA_ENTRY));
        }
        dwBlockOffset++;
    }
...
int FFUImage::FFUToRawProgram(const char* szFFUName, const char* szImageFile) {
    ...
    const char* szFFUFilePtr = strrchr(szFFUName, '\\');
    std::string szFFUFile;
    if (szFFUFilePtr != NULL) {
        szFFUFile = szFFUFilePtr + 1;
    } else {
        szFFUFile = szFFUName;
    }
    status = DumpRawProgram(szFFUFile.c_str(), szImageFile);
    if (status != ERROR_SUCCESS) {
        CloseFFUFile();
        return status;
    }
...
int FFUImage::PreLoadImage(const char* szFFUPath) {
    int status = ERROR_SUCCESS;
    OvlRead.hEvent = CreateEvent(NULL, FALSE, 0, NULL);
    OvlWrite.hEvent = CreateEvent(NULL, FALSE, 0, NULL);
...
int FFUImage::SplitFFUBin(const char* szPartName, const char* szOutputFile) {
    DWORD bytesRead = 0;
    ...
    for (i = 0; i < 128; i++) {
        ...
        if (strcmp(szPartName, "all") == 0) {
            // Ignore partitions without name
            if (strcmp(GptEntries[i].part_name, "") == 0) {
                continue;
            }
            SplitFFUBin(GptEntries[i].part_name, szOutputFile);
        } else if (strcmp(GptEntries[i].part_name, szPartName) == 0) {
    ...
    if ((i == 128) || (strcmp(GptEntries[i].part_name, "CrashDump") == 0)) return ERROR_SUCCESS;

    sprintf_s(szNewFile, sizeof(szNewFile), "%s\\%s.bin", szOutputFile, GptEntries[i].part_name);
    ...
    for (i = 0; i < FFUStoreHeader.dwWriteDescriptorCount; i++) {
        ...
        if (((diskOffset / 512) >= start_sector) && ((diskOffset / 512) <= end_sector)) {
            ...
            if (hImage == INVALID_HANDLE_VALUE) {
                LINFO("FFUImage::SplitFFUBin", "正在导出文件: %s", szNewFile);
                hImage = CreateFileA(szNewFile,
                    GENERIC_WRITE | GENERIC_READ,
                    0,    // We want exclusive access to this disk
                    NULL,
                    CREATE_ALWAYS,
...
int FFUImage::CloseFFUFile(void) {
    if (OvlRead.hEvent != NULL && OvlRead.hEvent != INVALID_HANDLE_VALUE) {
        CloseHandle(OvlRead.hEvent);
    }
    if (OvlWrite.hEvent != NULL && OvlWrite.hEvent != INVALID_HANDLE_VALUE) {
        CloseHandle(OvlWrite.hEvent);
    }
    if (hFFU != INVALID_HANDLE_VALUE) {
        CloseHandle(hFFU);
    }
    hFFU = INVALID_HANDLE_VALUE;
    return ERROR_SUCCESS;
